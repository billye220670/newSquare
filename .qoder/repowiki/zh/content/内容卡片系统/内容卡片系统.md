...
## 更新摘要
**变更内容**
- 视频卡片从单一展示升级为轮播功能，支持多视频内容的左右滑动浏览
- 新增 VideoSlide 子组件，负责单个视频轮播项的渲染与交互
- 详情页中的视频轮播功能得到完整保留，支持与主页面轮播镜像
- 底部品牌条的云下载按钮状态机得到增强，支持视频下载状态管理
- 新增视频轮播的指示器点阵，提供更好的导航体验
...
## 简介
NewSquare 是一个以内容卡片为核心的移动端信息流应用，围绕"内容卡片系统"构建了六大可复用卡片组件：视频卡片、杂志封面卡片、单品故事卡片、空间封面卡片、列表卡片和案例画廊卡片。系统通过统一的数据池与样式体系，为每类卡片提供一致的视觉语言与交互体验，并支持点击卡片进入详情页的全屏展开动画。

**重大更新**：视频卡片现已从单一展示升级为轮播功能，支持多视频内容的无缝切换浏览，为用户提供更加丰富的动态内容体验。
...
- 视频卡片：**轮播式视频展示**，支持多视频内容的左右滑动浏览，内置自动播放与暂停控制
...
### 视频卡片（VideoCard）- **重大更新**
**设计理念**
- 以轮播为核心的动态内容展示，支持多视频内容的无缝切换
- 通过 Swiper 实现流畅的左右滑动体验，内置指示器点阵
- 通过 IntersectionObserver 控制自动播放与暂停，优化性能
- 提供静音切换按钮，尊重移动端用户体验

**数据结构**
- VIDEO_URLS：包含多个视频地址与海报图
- VIDEO_CONTENT：卡片元数据数组（小标、标题、副标题、品牌、描述、CTA、图标与背景）

**Props**
- idx：索引，用于从数据池循环取值
- onOpen：点击回调，参数为 origin 与 payload
- downloads：视频下载状态管理对象
- startDownload：开始下载回调函数
- togglePauseResume：暂停/继续下载回调函数

**交互行为**
- 轮播式浏览多个视频内容
- 首次进入视口尝试播放当前视频，离开视口暂停
- 点击任意视频进入详情页，携带 variant: 'video' 与 payload
- 支持云下载状态管理，包括排队、准备中、下载中、暂停、完成等状态
...
```mermaid
flowchart TD
Start(["VideoCard 渲染"]) --> CreateSlides["创建轮播项<br/>聚合 VIDEO_CONTENT 与 VIDEO_URLS"]
CreateSlides --> InitSwiper["初始化 Swiper 轮播"]
InitSwiper --> IO["IntersectionObserver 监听"]
IO --> IsIn{"是否进入视口?"}
IsIn --> |是| TryPlay["尝试播放当前视频"]
IsIn --> |否| Pause["暂停播放"]
Click["点击视频"] --> Open["onOpen(origin, payload)"]
Open --> Detail["进入详情页"]
Detail --> DetailCarousel["详情页视频轮播"]
DetailCarousel --> SameUI["镜像主页面轮播 UI"]
```

**图表来源**
- [App.jsx:555-592](file://src/App.jsx#L555-L592)
- [App.jsx:888-935](file://src/App.jsx#L888-L935)
...
### 详情页视频轮播（DetailVideoCarousel）
**设计理念**
- 与主页面视频轮播完全镜像，提供一致的用户体验
- 支持从任意初始索引开始播放，保持状态同步
- 保留完整的品牌条与下载状态管理功能

**数据结构**
- slides：视频轮播项数组，包含完整元数据
- initialIndex：初始激活的轮播项索引
...
**章节来源**
- [App.jsx:631-689](file://src/App.jsx#L631-L689)
...
**章节来源**
- [App.jsx:692-715](file://src/App.jsx#L692-L715)
...
**章节来源**
- [App.jsx:718-751](file://src/App.jsx#L718-L751)
...
**章节来源**
- [App.jsx:754-805](file://src/App.jsx#L754-L805)
...
- 图片轮播：swiper（用于故事卡片、案例画廊、个人中心最近浏览、**视频卡片轮播**）
- 字体与图标：lucide-react
...
- **轮播优化**：Swiper 轮播仅在当前激活的视频项播放，其他视频自动暂停，避免多视频同时播放
...
- **轮播滑动异常**：确认 Swiper 初始化参数正确，检查 slidesPerView 和 speed 配置
...
- **视频下载状态异常**：检查 downloads 对象的状态机逻辑，确认状态转换的完整性
...
## 结论
NewSquare 的内容卡片系统通过统一的数据结构、样式规范与交互流程，实现了高复用性与强一致性。**经过重大升级的视频卡片**现在支持轮播功能，为用户提供了更加丰富和流畅的动态内容体验。六大卡片组件覆盖了从轮播视频到静态封面、从商品导流到空间展示的多种场景。通过详情页的统一入口与动画，系统在移动端提供了流畅的沉浸式体验。未来可在保持现有 API 稳定的前提下，扩展更多卡片类型与交互能力。
...
- **VideoCard（更新）**
  - props: { idx, onOpen, downloads, startDownload, togglePauseResume }
  - payload 示例字段：eyebrow, title, sub, heroImg, brand, desc, icon, iconBg, cta, slides[], initialIndex

- **VideoSlide（新增）**
  - props: { item, active, onOpen, downloads, startDownload, togglePauseResume, slides, slideIndex }
  - item: 单个视频轮播项的完整数据
  - active: 是否为当前激活的轮播项

- **DetailVideoCarousel（新增）**
  - props: { slides, initialIndex, downloads, startDownload, togglePauseResume }
  - slides: 视频轮播项数组
  - initialIndex: 初始激活的轮播项索引
...
- DetailOverlay
  - props: { detail, onClose, downloads, startDownload, togglePauseResume }
  - detail: { origin, variant, payload }
  - onClose: () => void

- 数据池字段说明（示例）
  - VIDEO_URLS: [{ url, poster }]（多个视频资源）
  - VIDEO_CONTENT: [{ eyebrow, title, sub, brand, desc, cta, icon, iconBg }]（多个视频元数据）
...
- **轮播组件**：如需轮播功能，参考 VideoCard 的实现方式，使用 Swiper 组件
- 注册调度：在 ORDER 与 BUILDERS 中注册新卡片类型
- 样式适配：在 index.css 中补充新卡片的样式与动画
- **轮播样式**：参考 .tcard-video-carousel 和 .video-carousel-swiper 的样式规范
