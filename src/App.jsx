...
/* ---------- 头部 ---------- */
function TopHeader({ hidden }) {
  const days = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六']
  const d = new Date()
  const dateStr = `${days[d.getDay()]}  ${d.getMonth() + 1}月${d.getDate()}日`
  return (
...
function makeBatch(startIndex, count = 5) {
  const arr = []
  for (let i = 0; i < count; i++) {
    const orderIdx = (startIndex + i) % ORDER.length
    const typeIdx = ORDER[orderIdx]
    const c = counters[typeIdx]++
    arr.push({ key: `${startIndex}-${i}`, type: typeIdx, idx: c })
  }
  return arr
}
...
export default function App() {
  const [items, setItems] = useState(() => makeBatch(0, 6))
  ...
  const loadMore = async () => {
    ...
    setItems(prev => [...prev, ...makeBatch(prev.length, 5)])
    if (items.length > 60) setHasMore(false)
  }

  return (
    <>
      <TopHeader hidden={hidden} />
      <main className="feed">
        {items.map(it => {
      ...
      <InfiniteScroll loadMore={loadMore} hasMore={hasMore}>
        {hasMore ? (
          ...
          <div className="loader">— 你已看完今日精选 —</div>
        )}
      </InfiniteScroll>
    </>
  )
